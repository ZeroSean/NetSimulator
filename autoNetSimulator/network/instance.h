...
//#define RESPONSE_OFFSET_TURNAROUND	(1500)	//車|∩e?角??車迆???車那?那㊣??米???辰?那㊣??㏒“us㏒?
#define RESPONSE_GUARD_TURNAROUND	(600)	//車|∩e?角??車迆???車那?那㊣??米???辰?那㊣??㏒“us㏒?
...
// BCN_LISTENER:?角足y赤????迆米?D?㊣那??㏒??a2谷?辰???Dtwr slot℅?℅?㊣?
// TWR_INITIATOR㏒o﹞⊿?e??
...
typedef struct {
    uint8   sessionID;			//赤????芍?～ID
    uint8   bcnFlag;
    uint8   clusterSlotNum;		//superframe?Dslot米?﹞???那y芍?
    uint8   clusterFrameNum;	//米㊣?～?迆米?﹞???米?米?superframe????
    uint16  clusterSelfMap;		//℅?谷赤米?beacon??????車Dbit map
    uint16  clusterNeigMap;
    uint16  slotSelfMap;		//℅?谷赤米?slot??????車?bit map
    uint16  nextSlotMap;		//??辰???3????D米?slot﹞????谷??
}BCNMan;

typedef struct {
    uint8 msgID;
    uint16 chldAddr;	//∩y車|∩e米?℅車?迆米?
    uint8  clusterLock;	//??℅???????米???那y?¯米1??那㊣㏒?0?辰??辰???2??????迆米?車|∩e芍?
    uint8  clusterSeat;	//??℅車?迆米?﹞???米?????
}msg_join_confirm;

typedef struct {
    uint16 flag;			//bitmap㏒????¯車???D??迄??range
    uint16 upPeriod;		//???車米??邦?迆那㊣??㏒?米ㄓ??ms
    uint16 ancAddr[4];		//??DDrange米??迄??米??﹞
    u8 		ancPosMask;	  	//??車|?迄??℅?㊣那米?車DD∫D?
    int32_t ancPos[4][3]; 	//??車|?迄??℅?㊣那㏒?mm

    int idist[4];			//mm
    u8 	idistMask;			//??車|?角角?米?車DD∫D?
    int avgDist[4];			//mm
    uint8 seqNum;			//twr D辰芍D??
    uint8 quaFac;			//?“???那芍?辰辰℅車---alarm
    uint8 posValid;			//℅?㊣那車DD∫D?bit[7], bit[2-0]-???¯那?米??????迄??D?o?
    uint8 warnFlag;			//bit[2:0]--bit[0]???1,bit[1]??車辰,bit[2]???豕
                            //bit[7:4]--bit[7]車DD∫D?,bit[6:4]?迄????那y
    int32_t pos[6];			//x,y,z℅?㊣那㏒?米ㄓ??mm, ?～辰???xyz??車辰車?車迆赤??﹞∩?那?㏒?o車辰???xyz車?車迆㊣㏒∩?㊣?㊣那??那y?Y
    uint8 pollMask;			//車DD∫?迄??那y??㏒o米赤4??㊣那那???車|??㊣那, ??4??㊣那那?那y??㏒?
    uint8 finalMask;		//??車|?迄????車|final㏒o米赤4??㊣那那???車|??㊣那, ??4??㊣那那?那y??㏒?
    uint8 finalCfm;			//??車|?迄????slot request米?豕﹞豕?
    uint64 gpollTxTime;
    uint64 gpollRxTime[4];
    uint64 pollTxTime[4];	//poll ﹞⊿?赤那㊣??
    uint64 pollRxTime[4];	//poll ?車那?那㊣??
    uint64 respTxTime;		//resp ﹞⊿?赤那㊣??
    uint64 respRxTime[4];	//resp ?車那?那㊣??
...
//那?米?米?D?㊣那???????芍11
typedef struct {
    uint16 srcAddr;		//D?㊣那??米??∩米??﹞
    uint16 clusMap;		//℅?谷赤谷豕㊣?米?cluster??車?bit map
    uint16 neigMap;		//?邦?∫谷豕㊣?米?cluster??車?bitmap
    uint16 slotMap;		//℅?谷赤米?twr slot??車?bit map
    uint16 receNum;		//?車那?∩?那y
    uint8  flag;		//bit0:㊣那那?芍??車???車車|∩e?DD豕辰a～邦角“???迆米?米?車|∩e㏒??∩豕?1?㊣那那??a1㏒??辰???車?車豕?赤???米??迆米?D豕辰a那?米????迆米?米?車|∩e㏒?
                        //bit1:tag?D㊣那那?那?﹞????a2a?角赤“D??迄??㏒?1那?0﹞?
}beacon_msg_log;
...
typedef struct
{
    ...
    uint32  instanceSleepTime_us;	//dw1000??豕??‘??米?那㊣??
...
/****************D?D-辰谷那㊣??赤?2?㊣?芍?*************************/
    ...
    //那㊣??D-米¯
    uint64 grpPollTx2RespTxDly_us;	//tag﹞⊿?赤group poll米?﹞⊿?赤response米?那㊣???車那㊣
    uint32 BCNfixTime32h;	//辰???BCN米??迆DW1000?D米?那㊣??
    uint32 SVCfixTime32h;	//SVC米?那㊣??
    //uint32 TWRfixTime32h;	//TWR米?那㊣??

    int fwto4PollFrame_sy;		//米豕∩ypoll??3?那㊣那㊣??
    int fwto4RespFrame_sy ; 	//米豕∩y?車那?response??米?3?那㊣那㊣??
    int fwto4FinalFrame_sy ; 	//米豕∩y?車那?final米?3?那㊣那㊣??

    uint32 fixedPollDelayAnc32h;	//1足?“米?poll米?角∩?～米??“﹞?那㊣??
    uint32 fixedFinalDelayAnc32h;	//1足?“米?final米?角∩?～米??“﹞?那㊣??

    uint32 fixedGuardDelay32h;	//1足?“米????角那㊣??
    uint32 fixedOffsetDelay32h;	//1足?“米???辰?那㊣??
//	uint32 fixedRespOffsetDelay32h;	//1足?“米???辰?那㊣??

    uint32 tagPollRxDelay_sy;	//﹞⊿?赤?～辰???o車㏒?poll??米?角∩米?∩車??那㊣??
    uint32 tagFinalRxDelay_sy;	//﹞⊿?赤?～辰???o車㏒?final??米?角∩米?∩車??那㊣??

    beacon_msg_log bcnlog[16];	//赤???2谷?迄????㊣赤㏒???㊣那?a??車|米?beacon????
    uint8  wait4ackNum;			//???車?車豕?赤????迆米?D豕辰a那?米?車|∩e米??迆米???那y

    uint8  idxOfAnc;			//?迄???迆TWR slot?D米???㊣那㏒“0 - 3㏒?
/*******************anchor㊣?芍?*******************************/
    //beacon frame
    BCNMan	bcnmag;				//beacon 1邦角赤?芍11
    //join confirmation frame
    msg_join_confirm jcofmsg;
    uint8   sampleNum;			//??赤???℅∩足???DD2谷?迄米?∩?那y赤3??
    uint8   fatherRef;			//?～辰?2???米?
    uint16	fatherAddr;			//2???米?米??﹞
    uint64	fatherRefTime;		//2???那㊣??
    uint32  svcStartStamp32h;	//SVC那㊣????米??a那?那㊣??
    uint32 	beaconTXTimeCnt;	//㊣?米?beacon﹞⊿?赤那㊣??
    //uint32 	beaconTXTimeh32_dw;	//DW beacon﹞⊿?赤那㊣??
    uint8   joinedNet;			//㊣那那?那?﹞??車豕?赤???
    uint8 	wait4type;			//米豕∩y車|∩e米?D??⊿角角D赤
    uint16	wait4ackOfAddr;		//米豕∩y???“米?米??﹞谷豕㊣?車|∩e㏒?豕??T㏒??辰谷豕???a0xffff

/*********************㊣那??㊣?芍?*******************************/
    TWRMan	twrmag;				//??DDtwr米?1邦角赤㊣?芍??芍11
    uint16 	slotMoniMap;
    uint8	occupySlot;			//twr那1車?米?twr slot????
    uint32	grpPollTxTime_us;	//group poll ﹞⊿?赤那㊣??
    uint64	grpPollTxStamp;		//group poll ﹞⊿?赤那㊣??∩芍--?迄車迆DW1000
    uint32  responseTxStamp32h;	//response﹞⊿?赤那㊣??∩芍

    uint32  refSlotStartTime_us;
    uint64  refSlotStartStamp;		//slot??車辰米?2????e那?那㊣??∩芍
/************************************************************/
    ...
    uint8 gatewayAnchor ; //set to TRUE = 1 if anchor address == 0
